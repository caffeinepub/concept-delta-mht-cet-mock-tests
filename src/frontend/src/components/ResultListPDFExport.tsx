...
export default function ResultListPDFExport({ usersWithAttempts }: ResultListPDFExportProps) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(25, 46, 91);
      const title = 'Test Results - User Score List';
      ...
      pdf.text('Name', margin + 3, yPosition + 7);
      pdf.text('Tests Taken', margin + contentWidth * 0.5, yPosition + 7);
      pdf.text('Avg Score', margin + contentWidth * 0.75, yPosition + 7);
      yPosition += 12;
      ...
      for (const [userProfile, attempts] of usersWithAttempts) {
        ...
        const rowIndex = usersWithAttempts.indexOf([userProfile, attempts]);
        if (rowIndex % 2 === 0) {
        ...
        pdf.setFontSize(9);
        pdf.text(userProfile.fullName, margin + 3, yPosition);
        pdf.text(attempts.length.toString(), margin + contentWidth * 0.5, yPosition);
        pdf.text(avgScore, margin + contentWidth * 0.75, yPosition);
        yPosition += 8;
      }
      ...
      toast.success('Result list PDF downloaded successfully!');
    } catch (error: any) {
