...
export default function PerTestScoreOnlyPDFExport({ test, usersWithAttempts }: PerTestScoreOnlyPDFExportProps) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(25, 46, 91);
      const title = `Score Report: ${test.name}`;
      ...
      const detailsText = `Subject: ${test.subject} | Total Participants: ${testParticipants.length}`;
      ...
      pdf.text('Score', margin + contentWidth * 0.75, yPosition + 7);
      yPosition += 12;
      ...
      sortedParticipants.forEach((participant, index) => {
        ...
        // Alternating row colors using deterministic index
        if (index % 2 === 0) {
        ...
        pdf.text(score, margin + contentWidth * 0.75, yPosition);
        yPosition += 8;
      });
      ...
      // Save PDF
      const fileName = `${test.name.replace(/[^a-z0-9]/gi, '_')}_Scores_${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(fileName);

      toast.success(`Score report for "${test.name}" downloaded successfully!`);
    } catch (error: any) {
